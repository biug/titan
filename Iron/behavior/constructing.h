...
public:
	...
	static TilePosition			FindBuildingPlacement(BWAPI::UnitType type, VBase * base, TilePosition near, int maxTries = 10);

	enum state_t {reachingLocation, waitingForMoney, constructing};

								Constructing(My<Terran_SCV> * pSCV, BWAPI::UnitType type, ConstructingExpert * pExpert = nullptr, TilePosition loc = TilePositions::None);
								~Constructing();
