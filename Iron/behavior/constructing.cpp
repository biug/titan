...
{
	int vab = 0;
	return canBuild(type, location, vab);
}
...
//	bw->drawBoxMap(center(SearchBoundingBox_TopLeft), center(SearchBoundingBox_BottomRight), Colors::Orange);
	...
	for (int x = SearchBoundingBox_TopLeft.x ; x <= SearchBoundingBox_BottomRight.x ; ++x)
	{
		...
		TilePosition targetCenter(target + dim / 2);
		...
		// 距离路口要>=7
		if (targetCenter.getApproxDistance((TilePosition)varea->HotCP()->Center()) >= 7)
			if (canBuild(type, target, verticalAdjacentBuildings))
			{
				// 应该离路口最近，离基地最远
				double score = - dist(targetCenter, TilePosition(varea->HotCP()->Center()))
							 + dist(targetCenter, TilePosition(ai()->Me().StartingBase()->Center()));
...
Constructing::Constructing(My<Terran_SCV> * pSCV, BWAPI::UnitType type, ConstructingExpert * pExpert, TilePosition loc)
	: Behavior(pSCV, behavior_t::Constructing), m_type(type), m_pExpert(pExpert), m_location(loc)
{
...
{CI(this);
	if (JustArrivedInState())
	{
		...
		double dist = (double)groundDist((Position)m_location, Agent()->Pos()) + 100;
		m_lastBuildFrame =(frame_t)(dist / (Agent()->Speed() * 0.717)) + 300 + ai()->Frame();
	...
	if (Agent()->GetStronghold() && ai()->Frame() > 24 * 60 * 8)
		if (m_pExpert)
			if (m_pExpert->CanChangeBuilder())
			{
				...
				if (pCloserWorker)
				{
					...
					pCloserWorker->ChangeBehavior<Constructing>(pCloserWorker, Type(), m_pExpert, m_location);
					m_pExpert = nullptr;
				///	bw << "bbb" << endl;
					return Agent()->ChangeBehavior<DefaultBehavior>(Agent());
				}
	...
	if (ai()->Frame() > m_lastBuildFrame)
	{
	///	bw << "ccc" << endl;
		if (m_pExpert) bw->sendText("wait building %s too long", UnitType(m_pExpert->GetID()).toString().c_str());
		return Agent()->ChangeBehavior<DefaultBehavior>(Agent());
	}
}
...
{CI(this);
	...
	if (ai()->Frame() - m_inStateSince > (buildingExists ? 1000 : 300))
	{
	///	bw << "ddd" << endl;
		return Agent()->ChangeBehavior<DefaultBehavior>(Agent());
	}
	...
	{
		ai()->GetMap().GetTile(TilePosition(Agent()->Pos()));
		for (auto & marine : me().Units(Terran_Marine))
		{
			if (marine.get() && marine.get()->Completed())
			{
				Position mPos = marine.get()->Pos();
				Position cPos = (Position)Location();
				if (mPos == cPos)
				{
					marine.get()->Move(me().StartingBase()->Center());
					bw->drawCircleMap(mPos, 16, Colors::Red, true);
				}
				else if (mPos.getDistance(cPos) < 32)
				{
					marine.get()->Move(mPos * 2 - cPos);
					bw->drawCircleMap(mPos, 16, Colors::Red, true);
				}
			}
		}
