...
{CI(this);
	...
	if (Agent()->GetArea() == me().StartingBase()->GetArea())
	{
		if (auto wall = me().GetVBase(0)->GetWall())
			if (wall->Size() >= 1)
			{
				const Tile & tile = ai()->GetMap().GetTile(TilePosition(Agent()->Pos()));
				Position wallTarget = (Position)wall->Center();
				if (ai()->GetVMap().InsideWall(tile) && Agent()->Pos().getApproxDistance(wallTarget) < 32)
				{
					if (ShouldPatrol()) return Agent()->Patrol(wallTarget);
					else return Agent()->Move(wallTarget);
				}
			}
	}
