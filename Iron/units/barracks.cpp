...
#include "../strategy/purpleswarm.h"
...
//	if (Units() >= 6) { m_priority = 0; return; }
	...
	if (ai()->GetStrategy()->Active<MassHydra>())
		if (Units() < (him().HasLurkers() ? 3 : 6))
			{ m_priority = 590; return; }

	if (ai()->GetStrategy()->Detected<Locutus>())
		if (Units() < 6)
		{
			m_priority = 605; return;
		}

	if (ai()->GetStrategy()->Detected<Purpleswarm>())
		if (Units() < 35)
		{
			m_priority = 605; return;
		}
	...
	if (GetWall())
		if (Completed())
		{
			if (Flying())
			{
				...
				else if (ai()->GetStrategy()->Active<MassHydra>())
				{
					TilePosition place = Constructing::FindBuildingPlacement(Type(), me().StartingVBase(), TilePosition(me().StartingBase()->Center()), 10);
					if (place != TilePositions::None)
						Land(place, check_t::no_check);
				}
				// 如果SKT激活，十分钟内
				if (ai()->GetStrategy()->Active<SKT>() && ai()->Frame() < 10000)
				{
					TilePosition landLoc = findSKTBarracksLand();
					bw->drawLineMap(Pos(), (Position)landLoc, Colors::Red);
					// 放在分矿
					Land((TilePosition)findNatural(me().StartingVBase())->BWEMPart()->Center(), check_t::no_check);
				}
			}
			else
			{
...
*/
					...
					if (!ai()->GetStrategy()->Active<Walling>())
					{
						if (TopLeft() == GetWall()->Locations()[IndexInWall()])
						{
							return Lift();
						}
						...
						if (me().CompletedBuildings(Terran_Barracks))
							if (!him().AllUnits(Zerg_Zergling).empty() || !me().CompletedUnits(Terran_Vulture))
								if (none_of(me().Buildings().begin(), me().Buildings().end(),
									[](const MyBuilding * b)
										{ return b->GetWall() && b->Completed() && b->Life() < b->MaxLife(); }))
									if (him().CreationCount(Zerg_Zergling) ||
										him().Buildings(Zerg_Extractor).empty() &&
										him().Buildings(Zerg_Hydralisk_Den).empty() &&
											!him().CreationCount(Zerg_Hydralisk))
							{
...
*/
	...
	{
		SetGuardInLoc();
		return ChangeBehavior<GuardLoc>(this, center(pNaturalArea->Top()));
	}
	// 探测到隐刀战术，设置枪兵GuardLoc
	if (ai()->GetStrategy()->Detected<Locutus>())
	{
		if (me().CompletedUnits(Terran_Marine) <= 6)
		{
			SetGuardInLoc();
			return ChangeBehavior<GuardLoc>(this, findMainGuard(me().StartingVBase()));
		}
		return;
	}
	// 出门切换SKT行为
	...
	{
		// 第8-13个枪兵守家
		if (me().CreationCount(Terran_Marine) >= 8 && me().CreationCount(Terran_Marine) <= 13)
		{
			SetGuardInLoc();
