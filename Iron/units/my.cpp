...
//					bw << "o" << endl;
		...
		bw->drawTextMap(this->Pos(), "%s(%s)", GetBehavior()->Name().c_str(), GetBehavior()->StateName().c_str());
		if (GetBehavior()->GetSubBehavior()) bw->drawTextMap(this->Pos().x, this->Pos().y + 15, "%s", GetBehavior()->GetSubBehavior()->Name().c_str());
		if (m_guardInLoc)
			bw->drawTextMap(this->Pos().x, this->Pos().y - 15, "GL");
	}
/*
	{
...
///	bw << NameWithId() << " land " << location << " --> " << (result ? "ok" : "ko") << endl;
	...
	if (Completed())
		if (double(Life()) / MaxLife() < MinLifePercentageToRepair() - 0.000001)
			if (RepairersCount() < MaxRepairers())
			{
				...
				if (GetWall())
				{
					if (me().LostUnitsOrBuildings(Terran_SCV) >= 4)
						if (!(Is(Terran_Bunker) && ai()->GetStrategy()->Active<MassHydra>()))
							return false;

					if (ai()->GetStrategy()->Active<Walling>() || ai()->GetStrategy()->Active<MassHydra>())
					{
						radius = 64*32;
