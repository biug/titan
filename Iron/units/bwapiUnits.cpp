...
{CI(this);
	if (Unit() && Unit()->getPlayer() == me().Player())
	...
	else
	{
		if (IsMyBuilding())
		{
			...
			for (Constructing * c : Constructing::Instances())
				if (Unit() && c->Agent()->Unit() == Unit()->getBuildUnit())
					pBuilder = c->Agent();
...
{CI(this);
	...
	if (Unit() && !Unit()->isInterruptible()) return false;
...
bool MyBWAPIUnit::AttackMove(Position pos, check_t checkMode)
{CI(this);
	//	if (GetBehavior()->IsAttacking()) 
	//	bw << NameWithId() << " attack! " << u->NameWithId() << endl;
	bool result = Unit()->attack(pos);
	OnCommandSent(checkMode, result, NameWithId() + " attack " + to_string(pos.x / 32) + "," + to_string(pos.y / 32));
	return result;
}
