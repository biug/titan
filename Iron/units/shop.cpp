...
void ExpertInConstructingAddon<Terran_Machine_Shop>::UpdateConstructingAddonPriority()
{
	...
	if (me().Buildings(Terran_Machine_Shop).size() == 1)
		if (ai()->GetStrategy()->Active<ProtossGreedy>() ||
			...
			if (me().Units(Terran_Vulture).size() < 15)
	...
	// 增加重工挂件的产能
	if (me().CompletedBuildings(Terran_Machine_Shop) == 0)
	{
		m_priority = 2000;
		return;
	}

	if (me().CompletedBuildings(Terran_Machine_Shop) == 1)
	{
		if (ai()->GetStrategy()->TimeToBuildSecondShop())
			m_priority = 1100;
		else
			m_priority = 0;
	...
	if (me().CompletedBuildings(Terran_Refinery) == 1)
	{
		m_priority = 0;
		return;
	}
	// 三重工出坦克，如果三气矿，出五个
	if (me().Buildings(Terran_Machine_Shop).size() < (me().CompletedBuildings(Terran_Refinery) > 2 ? 5 : 3))
	{
		m_priority = 10000 / (me().Buildings(Terran_Machine_Shop).size() * 10 + 1);
...
void ExpertInUpgraging<UpgradeTypes::Enum::Ion_Thrusters>::UpdateResearchingPriority()
{
	...
	if (ai()->GetStrategy()->Active<MassHydra>())
		if (me().UnitsBeingTrained(Terran_Vulture) +
			...
			< me().CompletedBuildings(Terran_Factory))
			{ m_priority = 0; return; }

	if (ai()->GetStrategy()->Detected<GoliathRush>())
		if (me().CompletedUnits(Terran_Siege_Tank_Tank_Mode) < 3)
			{ m_priority = 0; return; }
...
{
	if (me().HasResearched(TechTypes::Tank_Siege_Mode)) { m_priority = 0; return; }

	if (ai()->GetStrategy()->Detected<CannonRush>() || ai()->GetStrategy()->Detected<Locutus>())
