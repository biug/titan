...
public:
	...
	int										GetUpgradLevel(UpgradeType type) const	{ return Player()->getUpgradeLevel(type); }
	bool									IsUpgrading(UpgradeType type) const		{ return Player()->isUpgrading(type); }
	...
	void									SetLastBase(const VBase * pBase);
...
private:
	void									UpdateScans();
	void									HandleMacroLoaction(Expert * e, const MacroItem & item);
	...
	Opening									m_opening;
	bool									m_onceSKT = false;
	bool									m_DTRushByeBye = false;
};
