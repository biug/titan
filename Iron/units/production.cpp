...
void Production::OnFrame(Opening & opening)
{
	// 检查人口
...
//			if (b->TaskCost().Nul() && !a->TaskCost().Nul()) return false;
	...
	int shopCount = 0;
	int factoryCount = 0;
	for (auto & unit : bw->self()->getUnits())
	{
		...
		if (unit->getType() == Terran_Factory)
		{
			if (unit->isCompleted())
			{
				++factoryCount;
	...
	if (shopCount > 1) shopCount = factoryCount;
	for (Expert * e : m_Experts)
		if (e->Ready())
			if (!e->Selected())
			{
				if (opening.IsAllEmpty())
				{
					if (e->PriorityLowerThanOtherExperts()) continue;
					//	if (!e->TaskCost().Nul() && (e->Priority() < lastPriorityToCheck)) break;
					// 如果优先级低，并且资源少于两倍，不造
					if (e->Priority() < lastPriorityToCheck &&
						5 * e->TaskCost().Minerals() > MineralsAvailable() &&
						(e->TaskCost().Gas() == 0 || 3 * e->TaskCost().Gas() > GasAvailable()))
						break;
				}
				else if (!((!opening.IsEmpty() && opening.CheckRightExpert(e)) || opening.CheckRightKeepExpert(e, true)))
					if (!(e->GetID() == Terran_Machine_Shop && opening.GetBuildMachineShop() && shopCount < factoryCount))
				{
					continue;
				}
