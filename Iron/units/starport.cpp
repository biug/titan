...
#include "../behavior/hangingBase.h"
...
//	if (me().StartingBase()->Center().y < ai()->GetMap().Center().y) { m_priority = 0; return; }
	...
	// 对神族/虫族不出隐飞
	if (him().IsZerg() || him().IsProtoss() || me().CompletedBuildings(Terran_Starport) == 0)
	{
		m_priority = 0;
		return;
	}
...
{
	if (me().CompletedBuildings(Terran_Starport) == 0) {
		m_priority = 0;
		return;
	}
	if (me().Units(Terran_Goliath).size() < 5*him().AllUnits(Protoss_Carrier).size())
		if (me().Units(Terran_Goliath).size() < 20)
			if (me().Bases().size() < 4)
			{
				m_priority = 0;
				return;
	...
	if (him().IsProtoss())
	{
		if (him().Units(Protoss_Dragoon).size() > 10)
		{
			m_priority = 590;
			return;
		}
...
//	if (me().StartingBase()->Center().y < ai()->GetMap().Center().y) { m_priority = 0; return; }
	...
	if (ai()->GetStrategy()->Active<TankAdvance>() || ai()->GetStrategy()->Active<GuardNatural>())
		if (me().Buildings(Terran_Control_Tower).size() > 0)
			if (me().MineralsAvailable() >= 200 && me().GasAvailable() >= 100)
				if (me().Units(Terran_Dropship).size() == 0)
				{
					m_priority = 800;
					return;
				}
...
void ExpertInConstructing<Terran_Starport>::UpdateConstructingPriority()
{
	...
	// 如果没升一攻，不造机场
	if (me().GetUpgradLevel(UpgradeTypes::Terran_Vehicle_Plating) == 0 && !me().IsUpgrading(UpgradeTypes::Terran_Vehicle_Plating))
	{
		m_priority = 0;
		return;
	}
	...
	{//{ m_priority = 0; return; }
		if (Buildings() == 0)//drop
		{
			...
			if (ai()->GetStrategy()->Active<ProtossGreedy>() ||
				him().AllUnits(Protoss_Dragoon).size() >= 2 * me().Units(Terran_Siege_Tank_Tank_Mode).size())
				if (me().Bases().size() < 3)
					{ m_priority = 0; return; }
...
//	return ChangeBehavior<Dropping1T>(this);//drop
	...
	// 对抗locutus，激活hangingbase
	if (ai()->GetStrategy()->Active<GuardNatural>() || ai()->GetStrategy()->Active<TankAdvance>())
	{
		return ChangeBehavior<HangingBase>(this);
	}
