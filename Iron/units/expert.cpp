...
string TrainingExpert::ToString() const
{
	...
	s += Started() ? "started " : Selected() ? "selected " : "unselected ";
...
//	OnFinished();
	if (Selected())
	{
		...
		else
		{
			...
			if (!pBuilder)
				if (this->GetSpecLocation() != TilePositions::None)
						for (auto & unit : me().Units(Terran_SCV))
							if (unit.get() && unit.get()->Completed() && unit.get()->GetBehavior()->IsWalking())
								if (!pBuilder || unit.get()->Pos().getApproxDistance((Position)this->GetSpecLocation()) < pBuilder->Pos().getApproxDistance((Position)this->GetSpecLocation()))
									pBuilder = unit.get()->As<Terran_SCV>();
...
//			m_buildingCost = TaskCost();
			m_builderChanges = 0;
			m_pBuilder = pBuilder;
			m_pBuilder->ChangeBehavior<Constructing>(m_pBuilder, m_tid, this, this->GetSpecLocation());
