		...
		else
		{
			...
			for (MyUnit * u : me().Units())
				if (u->Completed() && !u->Loaded())
						...
						if (!u->GuardInLoc())
					{
						...
						if ((dist_unit_wall > 1*32) && (dist_unit_wall < 7*32))
						{
							...
							if (ai()->GetVMap().InsideWall(tile))
							{
								if (u->GetBehavior()->IsScouting() ||
									...
									pUnitShouldGetOut = u;
								if (u->GetBehavior()->IsConstructing() && me().Bases().size() == 2 && me().CompletedBuildings(Terran_Command_Center) == 1)
									pUnitShouldGetOut = u;
							}
							else
							{
			...
			///	if (pUnitShouldGetOut)  bw << "let " << pUnitShouldGetOut->NameWithId()  << " get out" << endl;
				...
				else
				{
					...
					if (pUnitShouldGetOut)
					{
						if (!pUnitShouldGetOut->GetBehavior()->IsConstructing())
						{
							pUnitShouldGetOut->ChangeBehavior<DefaultBehavior>(pUnitShouldGetOut);
						}
					}
				}
...
void Wall::MapSpecific()
{
	...
	if (!(him().IsProtoss() || him().IsTerran()))
	{
		...
		if (bw->mapHash() == "ba2fc0ed637e4ec91cc70424335b3c13e131b75a")	// Aztec
		{
			MapSpecificApply(89,7,		81,4,		81,0,		81, 2);
			MapSpecificApply(110,117,	106,121,	105,119,	105, 117);
			MapSpecificRemove(8,67);
			MapSpecificRemove(12,35);
		}
		...
		if (bw->mapHash() == "83320e505f35c65324e93510ce2eafbaa71c9aa1")	// Fortress
		{
			MapSpecificApply(55,120,		57,120,		57,118);
		}
